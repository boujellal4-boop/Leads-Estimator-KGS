...
conv_percent = colA.number_input(
...
# Interactive visualization next to conversion input (gauge)
fig_conv = go.Figure(go.Indicator(
    mode='gauge+number',
    value=conv_percent,
    gauge={'axis': {'range': [0, 100]}, 'bar': {'color': '#3b82f6'}, 'steps': [
...
colB.plotly_chart(fig_conv, use_container_width=True)
...
# Updated technologies list: split addressable into high-end and mid-end
technologies = [
...
if st.button('Start Estimation'):
    if len(st.session_state['persons']) == 0:
        ...
        with st.spinner('Estimating...'):
            ...
            df['Estimate'] = estimates_effective  # includes count & conversion
